...
def render_video(model_path, iteration, views, gaussians, pipeline, background):
    ...
    for idx, pose in enumerate(tqdm(generate_ellipse_path(views,n_frames=600), desc="Rendering progress")):
        ...
        view.full_proj_transform = (view.world_view_transform.unsqueeze(0).matmul(view.projection_matrix.unsqueeze(0))).squeeze(0)
        view.camera_center = view.world_view_transform.inverse()[3, :3].cuda()
        print("After load view",memory_usage())

        rendering = render(view, gaussians, pipeline, background)["render"]
        torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
        print("Render",memory_usage())
    ...
    print("Unload view",memory_usage())
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, video: bool, circular:bool, radius: float, args):
    torch.cuda.memory._record_memory_history(max_entries=7000)
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree)
        # print(memory_usage())
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, load_vq= args.load_vq, is_train=False)
        # print(memory_usage())
        ...
        print("Finish",memory_usage())
    torch.cuda.memory._dump_snapshot("lightgaussian.pickle")
