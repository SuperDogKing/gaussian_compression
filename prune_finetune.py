    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if iteration in args.prune_iterations:
                ...
                # 这个important_score 直接乘上了最大的体积，会导致把背景信息突出
                elif args.prune_type == "max_v_important_score":
                    v_list = imp_list * torch.max(gaussians.get_scaling, dim=1)[0]
                    gaussians.prune_gaussians(
                        (args.prune_decay**i) * args.prune_percent, v_list
                    )
                elif args.prune_type == "only_volumn":
                    v_list = calculate_v_imp_score(gaussians, gaussian_list, args.v_pow)
                    gaussians.prune_gaussians(
                        (args.prune_decay**i) * args.prune_percent, v_list
                    )
                ...
                print("Pruned")
                torch.cuda.memory._dump_snapshot("gaussian_splatting_prune.pickle")
