      ...
      while true; do
        ...
        if [[ -n $gpu_id ]]; then
          ...
          CUDA_VISIBLE_DEVICES=$gpu_id nohup python prune_finetune.py \
            -s "/projects/bcjw/xcui1/data/$arg" \
            -m "/projects/bcjw/xcui1/code/LightGaussian_2/output/${arg}" \
            --eval \
            --port $port \
            --iteration 1 \
            --prune_iterations 1 \
            --start_pointcloud "/projects/bcjw/xcui1/code/LightGaussian_2/output/${run_args}/point_cloud/iteration_30000/point_cloud.ply" \
